...
lf.schema.Builder.prototype.checkForeignKeyValidity_ = function(builder) {
  ...
  fkSpecArray.forEach(function(specs) {
    ...
    // TODO(sowmyasb): verify that this works for cross column primary keys
...
lf.schema.Builder.prototype.setParentForeignKeys_ = function(tableSchema) {
  ...
  this.tableBuilders_.getValues().forEach(function(builder) {
    builder.getFkSpecs().filter(function(foreignKeySpec) {
      if (foreignKeySpec.parentTable == tableSchema.getName()) {
        referencingKeys.push(foreignKeySpec);
      }
    });
  });
...
lf.schema.Builder.prototype.finalize_ = function() {
  if (!this.finalized_) {
    ...
    this.tableBuilders_.getValues().forEach(this.checkForeignKeyChain_, this);
    this.tableBuilders_.getKeys().forEach(this.connectParentChildren_, this);
    this.schema_.tables().forEach(this.setParentForeignKeys_, this);
    this.checkFkCycle_();
...
/**
 * Sets parent/child relationship among tables by foreign key references.
 * @param {string} localTableName The table on which
 *     foreign key checks are to be performed.
 * @private
 **/
lf.schema.Builder.prototype.connectParentChildren_ = function(localTableName) {
  var builder = /** @type {!lf.schema.TableBuilder} */ (
      this.tableBuilders_.get(localTableName));
  builder.getFkSpecs().forEach(
      /** @param {!lf.schema.ForeignKeySpec} spec */
      function(spec) {
        var localSchema = this.schema_.tables_.get(localTableName);
        var parentSchema = this.schema_.tables_.get(spec.parentTable);
        localSchema[spec.childColumn].setParent(
            parentSchema[spec.parentColumn]);
        var childrenColumns =
            parentSchema[spec.parentColumn].getChildren() || [];
