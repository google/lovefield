...
lf.schema.Builder.prototype.finalize_ = function() {
  if (!this.finalized_) {
    this.tableBuilders_.forEach(
        /** @this {!lf.schema.Builder}*/ function(builder, name) {
...
lf.schema.Builder.prototype.checkCycleUtil_ = function(graphNode, nodeMap) {
  if (!graphNode.visited) {
    ...
    graphNode.edges.forEach(/** @this {!lf.schema.Builder}*/ function(edge) {
...
lf.schema.Builder.prototype.connect = function(opt_options) {
  ...
  return this.db_.init(opt_options).then(
      function(db) {
        this.connectInProgress_ = false;
        return db;
      }.bind(this),
      function(e) {
        this.connectInProgress_ = false;
        // TODO(arthurhsu): Add a new test case to verify that failed init call
        // allows the database to be deleted since we close it properly here.
        this.db_.close();
        throw e;
      }.bind(this));
};
