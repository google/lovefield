...
lf.schema.TableBuilder.ForeignkeySpec = function(rawSpec, name) {
  ...
  if (array.length != 2) {
    throw new lf.Exception(
        ...
        'Foreign key {' + name + '} does not have a ' +
        'valid form \'table.column\'');
  }
...
lf.schema.TableBuilder.prototype.addForeignKey = function(name, rawSpec) {
  ...
  if (spec.action == lf.ConstraintAction.CASCADE &&
    ...
    throw new lf.Exception(
        lf.Exception.Type.SYNTAX,
        'Lovefield allows only immediate ' +
        'evaluation of cascading constraints');
  }

  if (!this.columns_.containsKey(spec.localColumn)) {
    throw new lf.Exception(
        lf.Exception.Type.SYNTAX,
        'Enter a valid column name for the' +
        'foreign key in local table');
  }
...
/** @export @return {!Array<lf.schema.TableBuilder.ForeignkeySpec>} */
