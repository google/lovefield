 ...
 */
lf.schema.TableBuilder.NULLABLE_TYPES_BY_DEFAULT = new lf.structs.Set([
...
lf.schema.TableBuilder.prototype.addForeignKey = function(fkName, rawSpec) {
  ...
  // TODO(sowmyasb): Tune the code to take uniqueness
  // of the index into account.
  this.addIndex(fkName, [spec.childColumn], false);
...
lf.schema.TableBuilder.prototype.checkNullableColumns_ = function(columns) {
  lf.structs.map.keys(this.indices_).forEach(
      function(indexName) {
        ...
        if (indexedColumnNames.size > 1 && nullableColumns.length > 0) {
...
lf.schema.TableBuilder.prototype.generateTableClass_ = function() {
  ...
  var tableClass = function() {
    ...
    var pk = that.indices_.has(that.pkName_) ?
        new lf.schema.Index(
    ...
    var unique = lf.structs.set.values(that.uniqueIndices_).
    ...
    this.constraint_ =
        new lf.schema.Constraint(pk, notNullable, that.getFkSpecs(), unique);
