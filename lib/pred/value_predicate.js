 ...
 * @param {number=} opt_bindIndex Default to -1. When provided, the predicate is
 *     "unbound", means that its actual value will be provided later by the user
 *     using bind() call.
 */
lf.pred.ValuePredicate = function(column, value, evaluatorType, opt_bindIndex) {
  ...
  /** @private {number} */
  this.bindIndex_ = goog.isDef(opt_bindIndex) ? opt_bindIndex : -1;
...
lf.pred.ValuePredicate.prototype.copy = function() {
  var clone = new lf.pred.ValuePredicate(
      this.column, this.value, this.evaluatorType, this.bindIndex_);
...
lf.pred.ValuePredicate.prototype.eval = function(relation) {
  goog.asserts.assert(!(this.value instanceof lf.Binder),
      'ValuePredicate shall already be bounded when eval() is called.');
...
lf.pred.ValuePredicate.prototype.bind = function(values) {
  if (this.bindIndex_ != -1) {
    if (goog.isDefAndNotNull(values) &&
        values.length &&
        values.length > this.bindIndex_) {
      this.value = /** @type {!T} */ (values[this.bindIndex_]);
    } else {
      throw new lf.Exception(lf.Exception.Type.SYNTAX,
          'Value is not bound at index ' + this.bindIndex_);
    }
  }
};
