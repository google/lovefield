...
lf.pred.ValuePredicate.prototype.eval = function(relation) {
  ...
  // If the relation being evaluated is the result of a join/cross-product
  // operation, the attributes of the rows being checked are prefixed by the
  // name of the originating table (normalized name).
  var attributeName = relation.isPrefixApplied() ?
      this.column.getNormalizedName() : this.column.getName();

  var entries = relation.entries.filter(function(entry) {
    return this.evaluatorFn_(
        entry.row.payload()[attributeName],
...
lf.pred.ValuePredicate.prototype.evalAsIn_ = function(relation) {
  ...
  var entries = relation.entries.filter(function(entry) {
    return evaluatorFn(entry.row.payload()[attributeName]);
  }, this);
...
/** @override */
lf.pred.ValuePredicate.prototype.evalRow = function(row) {
  return this.evaluatorFn_(
      row.payload()[this.column.getName()],
      this.value) != this.isComplement_;
};
