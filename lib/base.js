...
lf.base.enableInspector_ = function(global) {
  var exportFn = function() {
    ...
    // The chrome.devtools API only supports passing back plain JS object or
    // JSON. The payload can contain non-plain JS objects like Date, and
    // therefore need to stringify before passing it back.
    return JSON.stringify(exportTask.execSync(), undefined, 2);
  };
