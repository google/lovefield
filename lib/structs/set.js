...
lf.structs.set.create = function(opt_iterable) {
  ...
  return (window.Set &&
      ...
      // Workaround Safari bug: Safari creates a set with one element if called
      // with new window.Set(undefined)
      (goog.isDef(opt_iterable) ?
          new window.Set(opt_iterable) : new window.Set()) :
      new lf.structs.SetPolyFill_(opt_iterable);
};
