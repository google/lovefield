...
lf.tree.map = function(original, mapFn) {
  ...
  original.traverse(function(node) {
    ...
    var lastStackedParent = copyParentStack[copyParentStack.length - 1];
    if (node.isLeaf()) {
      if (node.isLastChild() && newNode.getParent() == lastStackedParent) {
        copyParentStack.pop();
      }
      nextParent = copyParentStack[copyParentStack.length - 1];
    }
  });
