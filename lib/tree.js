...
lf.tree.map = function(original, mapFn) {
  ...
  original.traverse(function(node) {
    ...
    if (node.getParent() == null) {
      ...
      copyParentStack[copyParentStack.length - 1].addChild(newNode);
    }
 ...
 * as a child.
 * Example: Calling insertNodeAt(n2, n6) would result in the following
 * transformation.
 *
 *        n1              n1
 *       /  \            /  \
...
lf.tree.replaceChainWithChain = function(oldHead, oldTail, newHead, newTail) {
  ...
  if (!goog.isNull(parentNode)) {
    parentNode.removeChild(oldHead);
    parentNode.addChild(newHead);
  }
