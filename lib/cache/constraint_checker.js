/**
 * @license
 * Copyright 2014 Google Inc. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
goog.provide('lf.cache.ConstraintChecker');

goog.require('goog.structs.Set');
goog.require('lf.Exception');
goog.require('lf.Global');
goog.require('lf.service');



/**
 * A helper class for performing various constraint checks.
 *
 * @constructor
 * @struct
 */
lf.cache.ConstraintChecker = function() {
  /** @private {!lf.index.IndexStore} */
  this.indexStore_ = lf.Global.get().getService(lf.service.INDEX_STORE);
};


/**
 * Finds if any row with the same primary key exists in the primary key index.
 * @param {!lf.schema.Table} table The table where the row belongs.
 * @param {!lf.Row} row The row whose primary key needs to checked.
 * @return {?number} The row ID of an existing row that has the same primary
 *     key as the input row, on null if no existing row was found.
 */
lf.cache.ConstraintChecker.prototype.findExistingRowIdInPkIndex = function(
    table, row) {
  var pkIndexSchema = table.getConstraint().getPrimaryKey();
  if (goog.isNull(pkIndexSchema)) {
    // There is no primary key for the given table.
    return null;
  }
  return this.findExistingRowIdInIndex_(pkIndexSchema, row);
};


/**
 * Finds if any row with the same index key exists in the given index.
 * @param {!lf.schema.Index} indexSchema The index to check.
 * @param {!lf.Row} row The row whose index key needs to checked.
 * @return {?number} The row ID of an existing row that has the same index
 *     key as the input row, on null if no existing row was found.
 * @private
 */
lf.cache.ConstraintChecker.prototype.findExistingRowIdInIndex_ = function(
    indexSchema, row) {
  var indexName = indexSchema.getNormalizedName();
  var indexKey = /** @type {!lf.index.Index.Key} */ (
      row.keyOfIndex(indexName));
  var index = this.indexStore_.get(indexName);

  var rowIds = index.get(indexKey);
  return rowIds.length == 0 ? null : rowIds[0];
};


/**
 * Finds all indices whose keys are unique, for the given table.
 * @param {!lf.schema.Table} table The table of interest.
 * @return {!Array.<!lf.schema.Index>}
 * @private
 */
lf.cache.ConstraintChecker.prototype.getUniqueIndices_ = function(table) {
  var uniqueIndices = [];
  var pkIndexSchema = table.getConstraint().getPrimaryKey();
  if (!goog.isNull(pkIndexSchema)) {
    uniqueIndices.push(pkIndexSchema);
  }
  return uniqueIndices.concat(table.getConstraint().getUnique());
};


/**
 * Checks that all the unique keys in the given set of rows are actually unique.
 * @param {!lf.schema.Table} table
 * @param {!Array.<!lf.Row>} rows
 * @throws {!lf.Exception}
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeysUnique = function(
    table, rows) {
  var uniqueIndices = this.getUniqueIndices_(table);
  for (var i = 0; i < uniqueIndices.length; i++) {
    this.checkUniqueKeyUnique_(uniqueIndices[i], rows);
  }
};


/**
 * Checks whether any unique key violation occurs as a result of inserting the
 * given set of rows.
 * @param {!lf.schema.Table} table The table where the rows belong.
 * @param {!Array.<!lf.Row>} rows The rows being inserted.
 * @throws {!lf.Exception}
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeysInsert = function(
    table, rows) {
  var uniqueIndices = this.getUniqueIndices_(table);
  for (var i = 0; i < uniqueIndices.length; i++) {
    this.checkUniqueKeyInsert_(uniqueIndices[i], rows);
  }
};


/**
 * Checks whether any unique key violation occurs as a result of updating the
 * given set of rows.
 * @param {!lf.schema.Table} table The table where the rows belong.
 * @param {!Array.<!lf.Row>} rows The rows being updated.
 * @throws {!lf.Exception}
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeysUpdate = function(
    table, rows) {
  var uniqueIndices = this.getUniqueIndices_(table);
  for (var i = 0; i < uniqueIndices.length; i++) {
    this.checkUniqueKeyUpdate_(uniqueIndices[i], rows);
  }
};


/**
 * Checks whether any not-nullable constraint violation occurs as a result of
 * inserting/updating the given set of rows.
 * @param {!lf.schema.Table} table The table where the rows belong.
 * @param {!Array.<!lf.Row>} rows The rows being inserted.
 * @throws {!lf.Exception}
 */
lf.cache.ConstraintChecker.prototype.checkNotNullable = function(table, rows) {
  var notNullable = table.getConstraint().getNotNullable();
  rows.forEach(function(row) {
    notNullable.forEach(function(column) {
      if (goog.isNull(row.payload()[column.getName()])) {
        throw new lf.Exception(
            lf.Exception.Type.CONSTRAINT,
            'Attempted to insert NULL value to non-nullable field ' +
            column.getNormalizedName());
      }
    }, this);
  }, this);
};


/**
 * Checks if any unique key violation occurs with respect to the given index, as
 * a result of inserting the given set of rows.
 * @param {!lf.schema.Index} indexSchema
 * @param {!Array.<!lf.Row>} rows
 * @throws {!lf.Exception}
 * @private
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeyInsert_ = function(
    indexSchema, rows) {
  var existingUniqueKey = null;
  var uniqueKeyAlreadyExists = rows.some(
      function(row) {
        var existingRowId = this.findExistingRowIdInIndex_(indexSchema, row);
        if (!goog.isNull(existingRowId)) {
          existingUniqueKey = row.keyOfIndex(indexSchema.getNormalizedName());
          return true;
        }

        return false;
      }, this);

  if (uniqueKeyAlreadyExists) {
    throw new lf.Exception(
        lf.Exception.Type.CONSTRAINT,
        'A row with key ' + existingUniqueKey + ' already exists ' +
        ' in ' + indexSchema.getNormalizedName());
  }
};


/**
 * Checks that the unique key corresponding to the given index in the given set
 * of rows is actually unique.
 *
 * @param {!lf.schema.Index} indexSchema
 * @param {!Array.<!lf.Row>} rows
 * @throws {!lf.Exception}
 * @private
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeyUnique_ = function(
    indexSchema, rows) {
  var uniqueKeys = new goog.structs.Set();
  rows.forEach(function(row) {
    var uniqueKey = row.keyOfIndex(indexSchema.getNormalizedName());
    uniqueKeys.add(uniqueKey);
  });

  if (uniqueKeys.getCount() < rows.length) {
    throw new lf.Exception(
        lf.Exception.Type.CONSTRAINT,
        'Unique key violation when inserting rows to ' +
        indexSchema.getNormalizedName());
  }
};


/**
 * Checks if any unique key violation occurs with respect to the given index, as
 * a result of updating the given set of rows.
 * @param {!lf.schema.Index} indexSchema
 * @param {!Array.<!lf.Row>} rows
 * @throws {!lf.Exception}
 * @private
 */
lf.cache.ConstraintChecker.prototype.checkUniqueKeyUpdate_ = function(
    indexSchema, rows) {
  var uniqueKeyModified = rows.some(function(updatedRow) {
    var existingRowId = this.findExistingRowIdInIndex_(indexSchema, updatedRow);
    return existingRowId != updatedRow.id();
  }, this);

  if (!uniqueKeyModified) {
    // Unique key is not modified so there is nothing to be checked.
    return;
  }

  if (rows.length > 1) {
    // Unique key is set to the same value for multiple rows. The query syntax
    // for update accepts only literals as values therefore all modified rows
    // will result in having the same value for the affeted column.
    throw new lf.Exception(
        lf.Exception.Type.CONSTRAINT,
        'Unique key violation when updating rows for ' +
        indexSchema.getNormalizedName());
  } else {
    this.checkUniqueKeyInsert_(indexSchema, [rows[0]]);
  }
};
