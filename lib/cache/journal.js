...
/**
 * Finds if any row with the same primary key exists.
 * It takes into account the changes that have happened within this journal and
 * are not reflected yet in the primary key index.
...
lf.cache.Journal.prototype.findExistingRowId_ = function(table, row) {
  var existingRowIdInInJournal = this.findExistingRowIdInJournal_(table, row);
  if (!goog.isNull(existingRowIdInInJournal)) {
    // An existing row with the same primary key was found in this journal.
    return existingRowIdInInJournal;
  }

  var existingRowIdInPkIndex = this.findExistingRowIdInPkIndex_(table, row);
  if (goog.isNull(existingRowIdInPkIndex)) {
    // Did not find any existing row, neither in the journal nor in the pkIndex.
    return null;
  }

  // A row was found in the pkIndex but not in the journal. Checking whether it
  // was just deleted.
  return this.isRowIdDeleted_(table.getName(), existingRowIdInPkIndex) ?
      null : existingRowIdInPkIndex;
};


/**
 * @param {string} tableName The name of the table where the row belongs.
 * @param {number} rowId The row ID to check.
 * @return {boolean} Whether the given row ID was deleted as part of this
 *     journal.
 * @private
 */
lf.cache.Journal.prototype.isRowIdDeleted_ = function(tableName, rowId) {
  var snapshot = this.snapshots_.get(tableName, null);
  return !goog.isNull(snapshot) && snapshot.containsKey(rowId) &&
      snapshot.get(rowId) == null;
};
...
lf.cache.Journal.prototype.findExistingRowIdInPkIndex_ = function(
  ...
  return goog.isDefAndNotNull(pkIndex.get(primaryKey)[0]) ?
      pkIndex.get(primaryKey)[0] : null;
};


/**
 * Finds if any row with the same primary key exists within this journal's logs.
 * @param {!lf.schema.Table} table The table where the row belongs.
 * @param {!lf.Row} row The row whose primary key needs to checked.
 * @return {?number} The row ID of an existing row that has the same primary
 *     key as the input row, on null if no existing row was found.
 * @private
 */
lf.cache.Journal.prototype.findExistingRowIdInJournal_ = function(table, row) {
  ...
  if (!goog.isNull(snapshot)) {
    ...
    var pkIndexName = pkIndexSchema.getNormalizedName();
    var primaryKey = row.keyOfIndex(pkIndexName);
    ...
    existingRow = goog.array.find(
        ...
        function(affectedRow) {
          ...
          var newRowPrimaryKey = affectedRow.keyOfIndex(pkIndexName);
          return primaryKey == newRowPrimaryKey;
        }, this);
  }

  return !goog.isNull(existingRow) ? existingRow.id() : null;
};
...
lf.cache.Journal.prototype.commit = function() {
  goog.asserts.assert(!this.committed_, 'Journal shall be committed only once');
