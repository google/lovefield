...
lf.backstore.BaseTx.prototype.mergeIntoBackstore_ = function() {
  ...
  diff.getKeys().forEach(
      ...
      function(tableName) {
        ...
        tableDiff.getDeleted().getValues().forEach(
            /**
             * @param {!lf.Row} row
             */
            function(row) {
              var promise = table.remove([row.id()]);
              promise.thenCatch(this.handleError_, this);
            });
        tableDiff.getAdded().getValues().forEach(
            /**
             * @param {!lf.Row} row
             */
            function(row) {
              var promise = table.put([row]);
              promise.thenCatch(this.handleError_, this);
            });
        tableDiff.getModified().getValues().forEach(
            /**
             * @param {!Array.<!lf.Row>} modification
             */
            function(modification) {
              var promise = table.put([modification[1]]);
              promise.thenCatch(this.handleError_, this);
            });
      }, this);
