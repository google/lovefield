 ...
 * @param {function(!IDBCursorWithValue)} loopFunc Function to run in cursor
 *     loop.
...
lf.backstore.IndexedDBRawBackStore.prototype.openCursorForWrite_ = function(
  ...
  return new goog.Promise(function(resolve, reject) {
    ...
    req.onsuccess = function(ev) {
      var cursor = /** @type {!IDBCursorWithValue} */ (req.result);
      if (cursor) {
        loopFunc(cursor);
        cursor.continue();
      } else {
...
lf.backstore.IndexedDBRawBackStore.prototype.transformRows_ = function(
  ...
  var loopFunc = function(cursor) {
    var row = lf.Row.deserialize(/** @type {!lf.Row.Raw} */ (cursor.value));
    rowFn(row);
  ...
  var loopFuncBundle = function(cursor) {
    var page = lf.backstore.Page.deserialize(
        /** @type {!lf.Row.Raw} */ (cursor.value));
