...
/**
 * Helper to read a value from Firebase ref to db.
 * @param {string} path
...
lf.backstore.Firebase.prototype.onChange_ = function(snapshot) {
  ...
  diffs.forEach(function(diff) {
    ...
    var toRemove = diff.getDeleted().getKeys();
    if (toRemove.length) {
...
lf.backstore.Firebase.prototype.generateDiff_ = function(snapshot) {
  var removedIds = new goog.structs.Set();
  var diffs = new goog.structs.Map();
  ...
  snapshot.forEach(function(child) {
    if (child.key() != '@rev') {
      ...
      if (!removedIds.contains(rowId)) {
        ...
        var table = this.tables_.get(diff.getName());
        if (!table.getData().containsKey(rowId)) {
          diff.add(new lf.Row(rowId, row['P']));
        } else {  // Must be an update.
          diff.modify([table.getSync([rowId])[0], new lf.Row(rowId, row['P'])]);
        }
      }
    }
  }.bind(this));
...
lf.backstore.Firebase.prototype.reloadTable = function(tableName) {
  ...
  this.db_.orderByChild('T').equalTo(tid).once('value', function(snapshot) {
    ...
    snapshot.forEach(function(rowSnapshot) {
      rows.push(new lf.Row(
          parseInt(rowSnapshot.key(), 10), rowSnapshot.val()['P']));
    });
