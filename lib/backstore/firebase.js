...
lf.backstore.Firebase.prototype.onChange_ = function(snapshot) {
  ...
  if (rev == null || rev == this.revision_) {
    return;
  }
...
lf.backstore.Firebase.prototype.generateDiff_ = function(snapshot) {
  ...
  var diffs = /** @type {!lf.structs.Map.<!lf.cache.TableDiff>} */ (
      lf.structs.map.create());
  ...
  snapshot.forEach(function(child) {
    if (child.key() != '@rev') {
      ...
      if (!removedIds.has(rowId)) {
        var row = child.val();
        var diff = /** @type {!lf.cache.TableDiff} */ (diffs.get(row['T']));
