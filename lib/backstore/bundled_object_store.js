 ...
 */
lf.backstore.BundledObjectStore = function(store, deserializeFn) {
...
lf.backstore.BundledObjectStore.prototype.getPageFromCache_ = function(pageId) {
  var cache = /** @type {!lf.cache.Cache} */ (
      lf.Global.get().getService(lf.service.CACHE));
  var range = lf.backstore.Page.getPageRange(pageId);
  var rows = cache.getRange(this.store_.name, range[0], range[1]);
