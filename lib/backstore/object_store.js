...
lf.backstore.ObjectStore.prototype.get = function(ids) {
  if (ids.length == 0) {
    return lf.Flags.IDB_GETALL_OPTIMIZATION && this.store_['getAll'] != null ?
        this.getAllBulk_() :
        this.getAllWithCursor_();
  }
...
lf.backstore.ObjectStore.prototype.getAllBulk_ = function() {
  return new goog.Promise(function(resolve, reject) {
    ...
    try {
      ...
      // 363: error reading IndexedDB objectStore.
...
/** @override */
lf.backstore.ObjectStore.prototype.remove = function(
    ids, disableClearTableOptimization) {
  var deleteByIdsFn = () => {
    var promises =
        ids.map((id) => this.performWriteOp_(() => this.store_.delete(id)));
  ...
  return new goog.Promise((resolve, reject) => {
    ...
    request.onsuccess = (ev) => {
      if (ids.length == 0 || ev.target.result == ids.length) {
        // Remove all
        this.performWriteOp_(() => this.store_.clear()).then(resolve, reject);
        return;
      }
