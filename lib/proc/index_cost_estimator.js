...
/**
 * The maximum percent of values that an lf.eval.Type.IN predicate can have to
 * still be considered for leveraging an index, with respect to the total number
 * of rows in the table.
 * For each one of the values a in index query will be performed, so the
 ...
 * @const {number}
 */
var IN_PREDICATE_PERCENT_LIMIT = 0.02;
...
lf.proc.IndexCostEstimator.prototype.getInPredicateValueLimit_ = function() {
  ...
  return Math.floor(rowIdIndex.stats().totalRows * IN_PREDICATE_PERCENT_LIMIT);
};
...
lf.proc.IndexCostEstimator.prototype.isCandidate_ = function(predicate) {
  ...
  if (predicate.evaluatorType == lf.eval.Type.IN &&
      predicate.value.length > this.getInPredicateValueLimit_()) {
    return false;
  }
