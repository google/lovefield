...
lf.proc.DeleteStep.prototype.exec = function(journal) {
  // TODO(user): Assert that this node has exactly one child.
  ...
  return this.getChildAt(0).exec(journal).then(goog.bind(
       ...
       */
      function(relation) {
        var rowIds = relation.entries.map(function(entry) {
          return entry.row.id();
        });
        journal.remove(this.table_, rowIds);
        return goog.Promise.resolve(lf.proc.Relation.createEmpty());
      }, this));
};
 ...
 */
lf.proc.ProjectStep = function(columns) {
...
lf.proc.ProjectStep.prototype.toString = function() {
  return 'project(' + this.columns.toString() + ')';
};
