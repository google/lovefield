...
lf.proc.PushDownSelectionsPass.prototype.pushDownJoinPredNodeRec_ =
  ...
  var shouldPushDownFn = function(child) {
    ...
    // Finding all tables that are involved in the subtree starting at this
    // child.
    var subtreeTables = new goog.structs.Set();
    lf.tree.getLeafNodes(child).forEach(function(tableAccessNode) {
      subtreeTables.add(tableAccessNode.table.getName());
    }, this);

    return subtreeTables.containsAll(selectNodeTables);
  };
