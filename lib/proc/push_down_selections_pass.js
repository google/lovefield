...
lf.proc.PushDownSelectionsPass.prototype.traverse_ = function(
  ...
  if (this.isCandidateNode_(node)) {
    ...
    if (!goog.isNull(newRoot)) {
      ...
      if (this.isCandidateNode_(newRoot) &&
        ...
        this.traverse_(
            /** @type {!lf.proc.LogicalQueryPlanNode} */ (newRoot),
            queryContext);
      }
    }
    return;
  ...
  node.getChildren().forEach(
      function(child) {
        this.traverse_(
            /** @type {!lf.proc.LogicalQueryPlanNode} */ (child), queryContext);
      }, this);
};
...
lf.proc.PushDownSelectionsPass.prototype.shouldSwapWithChild_ = function(
    queryContext, node) {
  var child = node.getChildAt(0);
  return child instanceof lf.proc.SelectNode &&
      // Don't swap with child if the child is a predicate corresponding to an
      // outer join.
      !(goog.isDefAndNotNull(queryContext.outerJoinPredicates) &&
