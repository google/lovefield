...
/**
 * Handles Sql queries that have left outer join. Does not support outer join
 * with where clause. TODO(sowmyasb): Stop throwing error once where clause
 * is handled.
 * @param {!lf.query.SelectContext} query
 * @param {boolean} stripValueInfo
 * @return {string}
...
lf.query.getFromListForOuterJoin_ = function(query, stripValueInfo) {
  ...
  var retrievedNodes = lf.tree.find(
      ...
      function(node) {
        // 361: toSql() does not support queries that involve where clause
        // with outer joins.
