 ...
 * @param {boolean} reverseOrder Retrieve the results in the reverse
...
lf.index.BTree.prototype.getAll_ = function(reverseOrder, limit, skip) {
  ...
  var offset = reverseOrder ? this.stats_.totalRows - count - skip : skip;
...
lf.index.BTree.prototype.getRange = function(
  ...
  if (!goog.isDef(opt_keyRanges) ||
    ...
    return this.getAll_(
        opt_reverseOrder || false,
        opt_limit || this.stats_.totalRows,
        opt_skip || 0);
  }

  var maxNumResults = (opt_limit || 0) + (opt_skip || 0);
  maxNumResults = (maxNumResults > 0 && !opt_reverseOrder) ? maxNumResults :
  ...
  sortedKeyRanges.forEach(function(range) {
    ...
    // Need to have two empty results to stop.
...
lf.index.BTreeNode_.prototype.getRange = function(keyRange, results) {
  ...
  return this.appendResults_(results, this.values_.slice(left, right + 1));
};
...
lf.index.BTreeNode_.prototype.appendResults_ = function(
  ...
  var toAppend = this.tree_.isUniqueKey() ?
