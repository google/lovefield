...
lf.index.BTree.prototype.cost = function(opt_keyRange) {
  // TODO(user): B-Tree should have better cost calculation.
...
/** @private {number} */
lf.index.BTreeNode_.nodeCount_ = 0;
...
lf.index.BTreeNode_.create = function(tree) {
  var node = new lf.index.BTreeNode_(lf.index.BTreeNode_.nodeCount_++, tree);
  return node;
};
 ...
 * @private
 */
lf.index.BTreeNode_.prototype.delete_ = function(key, parentPos) {
  var pos = this.searchKey_(key);
  if (!this.isLeaf_()) {
    var index = (this.keys_[pos] == key) ? pos + 1 : pos;
    if (this.children_[index].delete_(key, index) || index != pos) {
      this.fix_();
    }
  ...
  if (this.isLeaf_()) {
    if (this.keys_[pos] == key) {
      if (opt_replace) {
        ...
        // TODO(user): handle duplicate keys
      }
    }
    this.keys_.splice(pos, 0, key);
    this.values_.splice(pos, 0, value);
