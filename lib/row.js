 ...
 * @param {number} id
 */
lf.Row.prototype.setRowId = function(id) {
  this.id_ = id;
};
...
/**
 * Combines two rows into a single row.
 ...
 *     the right operand.
 * @return {!lf.Row} The combined row.
 */
lf.Row.combineRows = function(
    leftRow, leftRowPrefix, rightRow, rightRowPrefix) {
  var result = {};
  result[leftRowPrefix] = leftRow.payload();
  result[rightRowPrefix] = rightRow.payload();

  return new lf.Row(-1, result);
};
