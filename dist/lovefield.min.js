...
lf.backstore.IndexedDB.prototype.createObjectStoresForTable_=function(a,b,c){a.objectStoreNames.contains(c.getName())||a.createObjectStore(c.getName(),{keyPath:"id"});c.persistentIndex()&&(c=c.getIndices(),c.forEach(function(c){c=c.getNormalizedName();if(!a.objectStoreNames.contains(c)){a.createObjectStore(c,{keyPath:"id"});c=b.objectStore(c);var e=new lf.index.IndexMetadata(lf.index.IndexMetadata.Type.BTREE),e=new lf.index.IndexMetadataRow(e);c.put(e.serialize())}},this))};
...
lf.backstore.Memory.prototype.initTable_=function(a){this.createTable_(a.getName());a.persistentIndex()&&(a=a.getIndices(),a.forEach(function(a){a=this.createTable_(a.getNormalizedName());if(!goog.isNull(a)){var c=new lf.index.IndexMetadata(lf.index.IndexMetadata.Type.BTREE),c=new lf.index.IndexMetadataRow(c);a.put([c])}},this))};lf.backstore.Memory.prototype.close=function(){};
...
lf.index.BTreeNode_.prototype.remove=function(a){this.delete_(a,-1);return this.isRoot_()?(a=this,1==this.children_.length&&(a=this.children_[0],a.parent_=null),a):this};lf.index.BTreeNode_.leftMostKey_=function(a){return a.isLeaf_()?a.keys_[0]:lf.index.BTreeNode_.leftMostKey_(a.children_[0])};lf.index.BTreeNode_.prototype.fix_=function(){this.keys_=[];for(var a=1;a<this.children_.length;++a)this.keys_.push(lf.index.BTreeNode_.leftMostKey_(this.children_[a]))};
lf.index.BTreeNode_.prototype.delete_=function(a,b){var c=this.searchKey_(a);if(!this.isLeaf_()){var d=this.keys_[c]==a?c+1:c;(this.children_[d].delete_(a,d)||d!=c)&&this.fix_()}else if(this.keys_[c]!=a)return!1;this.keys_.length>c&&this.keys_[c]==a&&(this.keys_.splice(c,1),this.isLeaf_()&&this.values_.splice(c,1));return this.keys_.length<lf.index.BTreeNode_.MIN_KEY_LEN_&&!this.isRoot_()?(this.steal_()||this.merge_(b),!0):!1};
...
lf.index.BTreeNode_.prototype.insert=function(a,b,c){var d=this.searchKey_(a);if(this.isLeaf_()){if(this.keys_[d]==a){if(c)return this.values_[d]=b,this;if(this.tree_.isUniqueKeyOnly())throw new lf.Exception(lf.Exception.Type.CONSTRAINT,"Duplicate key not allowed");}this.keys_.splice(d,0,a);this.values_.splice(d,0,b);return this.keys_.length==lf.index.BTreeNode_.MAX_COUNT_?this.splitLeaf_():this}d=this.keys_[d]==a?d+1:d;a=this.children_[d].insert(a,b,c);a.isLeaf_()||1!=a.keys_.length||(this.keys_.splice(d,
0,a.keys_[0]),a.children_[1].parent_=this,a.children_[0].parent_=this,this.children_.splice(d,1,a.children_[1]),this.children_.splice(d,0,a.children_[0]));return this.keys_.length==lf.index.BTreeNode_.MAX_COUNT_?this.splitInternal_():this};
...
c+1)));return d};lf.index.BTreeNode_.serialize=function(a){for(var b=[];a;){var c=[a.keys_,a.values_];b.push(new lf.Row(a.id_,c));a=a.next_}return b};lf.index.BTreeNode_.deserialize=function(a,b){for(var c=a.map(function(a){var c=new lf.index.BTreeNode_(a.id(),b);c.keys_=a.payload()[0];c.values_=a.payload()[1];return c}),d=0;d<c.length-1;++d)lf.index.BTreeNode_.associate_(c[d],c[d+1]);return 1<c.length?lf.index.BTreeNode_.createInternals_(c[0]):c[0]};
