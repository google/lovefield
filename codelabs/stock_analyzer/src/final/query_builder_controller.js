...
QueryBuilderController.prototype.startObserving_ = function() {
  this.lovefieldService_.observeStockClosingPrices(
      function(changes) {
        var data = changes[0].object;
        this.updateStockGraph_(data);
      }.bind(this));
  this.lovefieldService_.observeSectorClosingPrices(
      function(changes) {
        var data = changes[0].object;
        this.updateSectorGraph_(data);
      }.bind(this));
};
...
QueryBuilderController.prototype.search = function() {
  ...
  // Instead of grappbing the results directly from the response, relying on
  // DB observers to update the UI.
