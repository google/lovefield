  ...
  var body = table.getColumns().map(function(col) {
    ...
    switch (col.getType()) {
      ...
      case lf.Type.DATE_TIME:
        if (col.isNullable()) {
          return lhs + 'goog.isNull(this.payload().' + col.getName() + ') ?\n' +
              '      null : this.payload().' + col.getName() + '.getTime();';
        }
 ...
 * TODO(dpapad): Expose lf.schema.Index from lovefield_externs.js?
 * @return {string}
 * @private
 */
...
CodeGenerator.prototype.getIndices_ = function(table) {
  ...
  // TODO(dpapad): See if this check is needed.
  if (table.getIndices()) {
    table.getIndices().forEach(function(indexSchema) {
      results.push(CodeGenerator.getIndexDefinition_(table, indexSchema, 4));
    }, this);
  }
...
CodeGenerator.prototype.generate = function(fileName, template) {
  ...
  // TODO(dpapad): Is this check needed?
